...
using namespace std;
  ...
  */
  ...
  class KeyExchanger {
    ...
    uint32_t wait_timer;
    const static uint16_t KEY_EXCHANGE_TIMEOUT = 10000;
    const static uint16_t GET_REQUEST_PERIOD = 100;
    const static uint16_t WAIT_TIME = 500;
    personKey pk;
    ...
    int update(/*int button1, int button2, int button3, int button4, char* output*/) {
        ...
        switch (state) {
            case GET_P_AND_G:
                request[0] = '\0';
                sprintf(request, "GET http://608dev-2.net/sandbox/sc/team33/final/key_exchange.py?other_user=%s&this_user=%s&get_type=gen_exchange HTTP/1.1\r\n", other_user, this_user);
            ...
            case POST_GA:
                ...
                // POST THE G_A
                {
                    ...
                    sprintf(request, "POST http://608dev-2.net/sandbox/sc/team33/final/key_exchange.py HTTP/1.1\r\n");
                    strcat(request, "Host: 608dev-2.net\r\n");
            ...
            case RECEIVE_GB:
                if (millis() - key_exchange_timer < KEY_EXCHANGE_TIMEOUT) {
                    if (millis() - last_get_request > GET_REQUEST_PERIOD) {
                      request[0] = '\0';
                      sprintf(request, "GET http://608dev-2.net/sandbox/sc/team33/final/key_exchange.py?other_user=%s&this_user=%s&get_type=exp_exchange HTTP/1.1\r\n", other_user, this_user);
                      strcat(request, "Host: 608dev-2.net\r\n"); //add more to the end
                      strcat(request, "\r\n");
                      do_http_request("608dev-2.net", request, response, OUT_BUFFER_SIZE, RESPONSE_TIMEOUT, true);
                    if (response[0] != '^') {
  ...
  class MessageHandler {
    private:
    const static uint16_t MESSAGE_HANDLING_PERIOD = 500;
    ...
    int update() {
      if (millis() - last_message_check > MESSAGE_HANDLING_PERIOD) {
        request[0] = '\0';
        sprintf(request, "GET http://608dev-2.net/sandbox/sc/team33/final/messages.py?token=12345&recipient=%s HTTP/1.1\r\n", this_user);
        strcat(request, "Host: 608dev-2.net\r\n"); //add more to the end
        strcat(request, "\r\n");
        do_http_request("608dev-2.net", request, response, OUT_BUFFER_SIZE, RESPONSE_TIMEOUT, true);
  ...
  class NewKeyExchangeHandler {
    private:
    const static uint16_t KEY_EXCHANGE_CHECK_PERIOD = 500;
    ...
    int update() {
      if (millis() - last_key_exchange_check > KEY_EXCHANGE_CHECK_PERIOD) {
        request[0] = '\0';
        sprintf(request, "GET http://608dev-2.net/sandbox/sc/team33/final/key_exchange.py?recipient=%s&get_type=new_key_exchange HTTP/1.1\r\n", this_user);
        strcat(request, "Host: 608dev-2.net\r\n"); //add more to the end
        strcat(request, "\r\n");
        do_http_request("608dev-2.net", request, response, OUT_BUFFER_SIZE, RESPONSE_TIMEOUT, true);
        Serial.println(response);
  ...
  KeyExchanger* key_exchanger_pointer = NULL;
  ...
  switch (main_state) {
    ...
    case 2:
      ...
      switch (key_exchanger_pointer->update()) {
        case -1:
          delete key_exchanger_pointer;
          key_exchanger_pointer = NULL;
          main_state = 0;
          break;
        case 1:
          delete key_exchanger_pointer;
...
void send_message(const char* user, const char* message, bigNum key) {
  char body[200];
  sprintf(body, "token=12345&sender=%s&recipient=%s&message=(%s, %llu)", USERNAME, user, message, key); //generate body, posting temp, humidity to server
  int body_len = strlen(body); //calculate body length (for header reporting)
  sprintf(request, "POST http://608dev-2.net/sandbox/sc/sgholla/final-project/messages.py HTTP/1.1\r\n");
  strcat(request, "Host: 608dev-2.net\r\n");
  strcat(request, "Content-Type: application/x-www-form-urlencoded\r\n");
  sprintf(request + strlen(request), "Content-Length: %d\r\n", body_len); //append string formatted to end of request buffer
  strcat(request, "\r\n"); //new line from header to body
