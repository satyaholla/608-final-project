...
def create_database():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS message_table (sender text, recipient text, message text, token int, read int, timing timestamp);''') # run a CREATE TABLE command
...
def request_handler(request):
    ...
    if request["method"] == "GET": #from receiver
        ...
        if not verify_token(recipient, token):
            conn.commit()
            conn.close()
            return '#'
